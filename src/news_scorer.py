...
COMMUNITY_TERMS = ["ask hn", "show hn", "hacker news", "discussion", "comments", "热议", "讨论"]
...
class NewsScoringService:
    ...
    def _classify(self, item: NewsItem, text: str) -> str:
        ...
        if item.source_type == "hackernews" and (_contains_any(text, COMMUNITY_TERMS) or lowered_title.startswith(QUESTION_PREFIXES)):
    ...
    def _source_score(self, item: NewsItem, reasons: list[str]) -> float:
        ...
        if source_type == "official_rss":
            ...
            reasons.append("来自 Hacker News 讨论")
    ...
    def _discussion_score(self, item: NewsItem, reasons: list[str]) -> float:
        if item.source_type != "hackernews":
            return 30.0
        points, comments = self._extract_hn_metrics(item)
        if points or comments:
            score = 42.0 + min(38.0, math.log1p(points) * 8.0 + math.log1p(comments) * 7.0)
            reasons.append(f"HN 讨论热度: {int(points)} points / {int(comments)} comments")
            return _clamp(score)
        reasons.append("HN 来源，具备基础社区讨论价值")
        return 58.0
    ...
    def _importance_score(
        ...
        if item.source_type == "hackernews" and item.raw_score >= 50:
            score += 8.0
        return _clamp(score)
    ...
    def _noise_penalty(self, item: NewsItem, text: str, category: str, warnings: list[str]) -> float:
        ...
        if lowered_title.startswith(QUESTION_PREFIXES) and item.source_type == "hackernews":
            penalty += 10.0
    ...
    def _extract_hn_metrics(self, item: NewsItem) -> tuple[float, float]:
        summary = item.summary or ""
        points_match = re.search(r"points\s*=\s*(\d+)", summary, flags=re.IGNORECASE)
        comments_match = re.search(r"comments\s*=\s*(\d+)", summary, flags=re.IGNORECASE)
        points = float(points_match.group(1)) if points_match else float(item.raw_score or 0)
